...
shakti_main(void *arg)
{
	...
	say("[patch_module] breakpoint installed, calling target_func() directly");

	target_fn = (void (*)(void))addr;
	target_fn();

	say("[patch_module] target_func() returned WITHOUT the breakpoint firing (unexpected)");
	return;
}
